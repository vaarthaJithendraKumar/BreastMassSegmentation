...
#identify GPU
# Device configuration (GPU optional)
...
history = model.fit(imgs_train, imgs_mask_train, batch_size=16, epochs=150, verbose=1, shuffle=True,
...
# Get test image filenames from the dataset folder
test_img_dir = os.path.join(name, 'Test', 'img')
if os.path.exists(test_img_dir):
    test_filenames = sorted(os.listdir(test_img_dir))
else:
    test_filenames = []

for idx, (image, image_id) in enumerate(zip(imgs_mask_test, imgs_id_test)):
    image = (image[:, :, 0] * 255.).astype(np.uint8)
    if idx < len(test_filenames):
        base = os.path.splitext(test_filenames[idx])[0]
        output_name = base + '_pred.png'
    else:
