...
# --- Create .npy files when run directly ---
if __name__ == '__main__':
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    NPY_DIR = os.path.join(BASE_DIR, 'UNets files')
    os.makedirs(NPY_DIR, exist_ok=True)

    dataset_name = 'CBIS-DDSM'

    # Training data
    train_path = os.path.join(BASE_DIR, dataset_name)
    create_train_data(train_path, dataset_name)
    # Move to UNets files/
    for f in [f'imgs_train_{dataset_name}.npy', f'imgs_mask_train_{dataset_name}.npy']:
        src = os.path.join(BASE_DIR, f)
        dst = os.path.join(NPY_DIR, f)
        if os.path.exists(src):
            os.replace(src, dst)

    # Test data
    test_path = os.path.join(BASE_DIR, dataset_name, 'Test')
    create_test_data(test_path, dataset_name)
    # Move to UNets files/
    for f in [f'imgs_test_{dataset_name}.npy', f'imgs_id_test_{dataset_name}.npy']:
        src = os.path.join(BASE_DIR, f)
        dst = os.path.join(NPY_DIR, f)
        if os.path.exists(src):
            os.replace(src, dst)

    print('All .npy files saved to:', NPY_DIR)
